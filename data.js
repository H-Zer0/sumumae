...
const SPEC_RISK_TRANSLATOR = {
    structure: {
        木造: {
            lifeRisk: '隣人の生活音（会話・くしゃみ）が丸聞こえ。断熱性が低く、夏は暑く冬は極寒。',
            parentConcern: '騒音トラブルや、冬のヒートショック・体調不良が心配。',
            severity: 'high'
        },
        軽量鉄骨: {
            lifeRisk: '隣人の生活音（会話・くしゃみ）が丸聞こえ。断熱性が低く、夏は暑く冬は極寒。',
    ...
    floor: {
        1: {
            lifeRisk: '防犯リスク最大。外から丸見え。下着が干せない。虫（G）の侵入率が非常に高い。',
            parentConcern: '空き巣、のぞき、不審者。親が最も反対するポイント。',
            severity: 'critical'
        },
        2: {
            lifeRisk: '防犯面では1階よりマシだが、虫の侵入リスクは残る。',
            parentConcern: '1階よりは安心だが、オートロックがあるとより良い。',
            severity: 'medium'
        },
        '3以上': {
            lifeRisk: '防犯面・虫の侵入リスクが低い。エレベーターなしだと買い物が大変。',
            parentConcern: '防犯面で安心。エレベーターの有無を確認。',
            severity: 'low'
        }
    },
    ...
    features: {
        ロフト: {
            lifeRisk: '夏は熱気が溜まり冷房が効かない。掃除が困難。ハシゴが面倒で結局使わなくなる。',
            parentConcern: '転落事故や、掃除が行き届かず不衛生になる心配。',
            severity: 'medium'
        },
        '3点ユニット': {
            lifeRisk: 'トイレが常に湿気る。友人が泊まった時に不便。掃除は楽だが、QOLは下がりやすい。',
            parentConcern: '湯船に浸かれないことによる健康管理への懸念。',
            severity: 'medium'
        },
        角部屋: {
            lifeRisk: '隣室と接する面が少なく静かだが、外気の影響を最も受けやすく、結露・カビが発生しやすい。',
            parentConcern: '光熱費の増大と、カビによる健康被害・退去費用の発生。',
            severity: 'medium'
        }
    },
